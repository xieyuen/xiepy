...
This file is adapted from scripts of MCDReforged Plugin Catalogue.
...
According to the GPL v3 license, all CI scripts,
namely all Python files in the `scripts` folder of the project root,
will be made open-source under the GPL v3 license.

Other Python file, including but not limited to what is located in
the `src` folder of the project root, will be under the LGPL v3 License.
"""
...
logger = get_logger("GitHub CLI (Pull Request CI)")

EXECUTABLE = "gh"
PR_NUMBER: str = os.environ.get("PR_NUMBER")
REPO = "xieyuen/xiepy".split("/")
COMMENT_SIGN: str = "<!-- xiepy CI -->"
...
def pr_update_or_comment(
    user: str,
    body: str,
    pr_number: str = PR_NUMBER,
    sign: str = COMMENT_SIGN,
) -> None:
...
def pr_label(
    ...
    """
    logger.info(
        f"Labeling PR: #{pr_number}, add_labels: {add_labels}, remove_labels: {remove_labels}",
    )
...
def check_contributor(pr_number: str = PR_NUMBER) -> tuple[Optional[str], bool]:
    ...
    try:
        ...
        author_login = result_json["data"]["repository"]["pullRequest"]["author"][
            "login"
        ]
        is_first_time = (
            result_json["data"]["repository"]["pullRequest"][
                "authorAssociation"
            ].upper()
