...
logger = get_logger("GitHub CLI (CI)")
...
def pr_label(add_labels: Optional[list[str]] = None, remove_labels: Optional[list[str]] = None,
...
def check_contributor(pr_number: str = PR_NUMBER) -> Tuple[Optional[str], bool]:
    ...
    try:
        ...
        is_first_time = result_json['data']['repository']['pullRequest'][
                            'authorAssociation'].upper() == 'FIRST_TIME_CONTRIBUTOR'
