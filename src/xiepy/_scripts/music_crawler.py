...
class Constants:
    ...
    platformMap = {
        ...
        for keys, v in (
            ...
            (("2", "q", "qq", "qqmusic", "qqyinyue", "qq音乐", "qq 音乐"), "qq"),
            (("3", "kg", "ku", "kou", "gou", "kugou", "酷狗"), "kugou"),
            (("4", "kw", "ko", "wo", "kuwo", "酷我"), "kuwo"),
            (("5", "b", "bd", "bu", "baidu", "百度"), "baidu"),
            (("6", "x", "xi", "xmly", "xmla", "ximalaya", "喜马拉雅"), "ximalaya"),
        )
        for k in keys
    }
...
class Crawler:
    ...
    def main(self):
        ...
        for platform in platforms:
            param = self.getParam(name=name, platform=platform)
        ...
        index = 1
        shouldIgnores = []
        for i, (platform, tits, diffAuthor) in enumerate(
            zip(platforms, titles, authors)
        ):
            if not diffAuthor:
        ...
        titles = [
            ...
            if ind not in shouldIgnores
            for i in item
        ]
        authors = [
            i
            for ind, item in enumerate(authors)
            if ind not in shouldIgnores
            for i in item
        ]
        urls = [
            i for ind, item in enumerate(urls) if ind not in shouldIgnores for i in item
        ]
...
if __name__ == "__main__":
    ...
    args = {key: value for item in sys.argv[1:] for key, value in [item.split("=")]}
    if "--path" in args:
